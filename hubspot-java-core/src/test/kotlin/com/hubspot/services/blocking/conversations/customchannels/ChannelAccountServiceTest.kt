// File generated from our OpenAPI spec by Stainless.

package com.hubspot.services.blocking.conversations.customchannels

import com.hubspot.client.okhttp.HubSpotOkHttpClient
import com.hubspot.models.conversations.customchannels.PublicChannelAccountEgg
import com.hubspot.models.conversations.customchannels.PublicChannelAccountStagingTokenUpdateRequest
import com.hubspot.models.conversations.customchannels.PublicChannelAccountUpdateRequest
import com.hubspot.models.conversations.customchannels.PublicDeliveryIdentifier
import com.hubspot.models.conversations.customchannels.channelaccounts.ChannelAccountCreateParams
import com.hubspot.models.conversations.customchannels.channelaccounts.ChannelAccountUpdateParams
import com.hubspot.models.conversations.customchannels.channelaccounts.ChannelAccountUpdateStagingTokenParams
import org.junit.jupiter.api.Disabled
import org.junit.jupiter.api.Test

internal class ChannelAccountServiceTest {

    @Disabled("Mock server tests are disabled")
    @Test
    fun create() {
        val client = HubSpotOkHttpClient.builder().accessToken("My Access Token").build()
        val channelAccountService = client.conversations().customChannels().channelAccounts()

        val publicChannelAccount =
            channelAccountService.create(
                ChannelAccountCreateParams.builder()
                    .channelId(0)
                    .publicChannelAccountEgg(
                        PublicChannelAccountEgg.builder()
                            .authorized(true)
                            .inboxId("inboxId")
                            .name("name")
                            .deliveryIdentifier(
                                PublicDeliveryIdentifier.builder()
                                    .type(PublicDeliveryIdentifier.Type.CHANNEL_SPECIFIC_OPAQUE_ID)
                                    .value("value")
                                    .build()
                            )
                            .build()
                    )
                    .build()
            )

        publicChannelAccount.validate()
    }

    @Disabled("Mock server tests are disabled")
    @Test
    fun update() {
        val client = HubSpotOkHttpClient.builder().accessToken("My Access Token").build()
        val channelAccountService = client.conversations().customChannels().channelAccounts()

        val publicChannelAccount =
            channelAccountService.update(
                ChannelAccountUpdateParams.builder()
                    .channelId(0)
                    .channelAccountId(0L)
                    .publicChannelAccountUpdateRequest(
                        PublicChannelAccountUpdateRequest.builder()
                            .authorized(true)
                            .name("name")
                            .build()
                    )
                    .build()
            )

        publicChannelAccount.validate()
    }

    @Disabled("Mock server tests are disabled")
    @Test
    fun list() {
        val client = HubSpotOkHttpClient.builder().accessToken("My Access Token").build()
        val channelAccountService = client.conversations().customChannels().channelAccounts()

        val page = channelAccountService.list(0)

        page.response().validate()
    }

    @Disabled("Mock server tests are disabled")
    @Test
    fun updateStagingToken() {
        val client = HubSpotOkHttpClient.builder().accessToken("My Access Token").build()
        val channelAccountService = client.conversations().customChannels().channelAccounts()

        val publicChannelAccountStagingToken =
            channelAccountService.updateStagingToken(
                ChannelAccountUpdateStagingTokenParams.builder()
                    .channelId(0)
                    .accountToken("accountToken")
                    .publicChannelAccountStagingTokenUpdateRequest(
                        PublicChannelAccountStagingTokenUpdateRequest.builder()
                            .accountName("accountName")
                            .deliveryIdentifier(
                                PublicDeliveryIdentifier.builder()
                                    .type(PublicDeliveryIdentifier.Type.CHANNEL_SPECIFIC_OPAQUE_ID)
                                    .value("value")
                                    .build()
                            )
                            .build()
                    )
                    .build()
            )

        publicChannelAccountStagingToken.validate()
    }
}
