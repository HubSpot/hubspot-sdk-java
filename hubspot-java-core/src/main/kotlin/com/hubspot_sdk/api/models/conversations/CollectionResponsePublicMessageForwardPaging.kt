...
private constructor(
    private val results: JsonField<List<PublicMessage>>,
    ...
    private constructor(
        ...
        @ExcludeMissing
        results: JsonField<List<PublicMessage>> = JsonMissing.of(),
     ...
     */
    fun results(): List<PublicMessage> = results.getRequired("results")
    ...
    @JsonProperty("results")
    @ExcludeMissing
    fun _results(): JsonField<List<PublicMessage>> = results
    ...
    class Builder internal constructor() {
        ...
        private var results: JsonField<MutableList<PublicMessage>>? = null
        ...
        fun results(results: List<PublicMessage>) = results(JsonField.of(results))

        /**
         * Sets [Builder.results] to an arbitrary JSON value.
         *
         * You should usually call [Builder.results] with a well-typed `List<PublicMessage>` value
         * instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun results(results: JsonField<List<PublicMessage>>) = apply {
            this.results = results.map { it.toMutableList() }
        }

        /**
         * Adds a single [PublicMessage] to [results].
         *
         * @throws IllegalStateException if the field was previously set to a non-list.
         */
        fun addResult(result: PublicMessage) = apply {
            results =
         ...
         */
        fun addResult(conversationsPublicConversations: ConversationsPublicConversationsMessage) =
            addResult(
                PublicMessage.ofConversationsPublicConversations(conversationsPublicConversations)
            )

        /** Alias for calling [addResult] with `PublicMessage.ofComment(comment)`. */
        fun addResult(comment: PublicComment) = addResult(PublicMessage.ofComment(comment))

        /** Alias for calling [addResult] with `PublicMessage.ofWelcome(welcome)`. */
        fun addResult(welcome: PublicWelcomeMessage) = addResult(PublicMessage.ofWelcome(welcome))

        /** Alias for calling [addResult] with `PublicMessage.ofAssignment(assignment)`. */
        fun addResult(assignment: PublicAssignmentMessage) =
            addResult(PublicMessage.ofAssignment(assignment))

        /**
         * Alias for calling [addResult] with
         * `PublicMessage.ofThreadStatusChange(threadStatusChange)`.
         */
        fun addResult(threadStatusChange: PublicThreadStatusChange) =
            addResult(PublicMessage.ofThreadStatusChange(threadStatusChange))

        /**
         * Alias for calling [addResult] with
         * `PublicMessage.ofThreadInboxChange(threadInboxChange)`.
         */
        fun addResult(threadInboxChange: PublicThreadInboxChange) =
            addResult(PublicMessage.ofThreadInboxChange(threadInboxChange))
