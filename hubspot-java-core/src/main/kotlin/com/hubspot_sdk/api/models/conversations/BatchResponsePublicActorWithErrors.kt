...
class BatchResponsePublicActorWithErrors
@JsonCreator(mode = JsonCreator.Mode.DISABLED)
private constructor(
    private val completedAt: JsonField<OffsetDateTime>,
    private val results: JsonField<List<PublicActor>>,
    ...
    private constructor(
        ...
        @ExcludeMissing
        results: JsonField<List<PublicActor>> = JsonMissing.of(),
     ...
     *   unexpectedly missing or null (e.g. if the server responded with an unexpected value).
     */
    fun results(): List<PublicActor> = results.getRequired("results")
     ...
     */
    @JsonProperty("results") @ExcludeMissing fun _results(): JsonField<List<PublicActor>> = results
    ...
    /** A builder for [BatchResponsePublicActorWithErrors]. */
    class Builder internal constructor() {

        private var completedAt: JsonField<OffsetDateTime>? = null
        private var results: JsonField<MutableList<PublicActor>>? = null
        ...
        @JvmSynthetic
        internal fun from(batchResponsePublicActorWithErrors: BatchResponsePublicActorWithErrors) =
            apply {
                completedAt = batchResponsePublicActorWithErrors.completedAt
                results = batchResponsePublicActorWithErrors.results.map { it.toMutableList() }
                startedAt = batchResponsePublicActorWithErrors.startedAt
                status = batchResponsePublicActorWithErrors.status
                errors = batchResponsePublicActorWithErrors.errors.map { it.toMutableList() }
                links = batchResponsePublicActorWithErrors.links
                numErrors = batchResponsePublicActorWithErrors.numErrors
                requestedAt = batchResponsePublicActorWithErrors.requestedAt
                additionalProperties =
                    batchResponsePublicActorWithErrors.additionalProperties.toMutableMap()
            }
        ...
        fun results(results: List<PublicActor>) = results(JsonField.of(results))

        /**
         * Sets [Builder.results] to an arbitrary JSON value.
         *
         * You should usually call [Builder.results] with a well-typed `List<PublicActor>` value
         * instead. This method is primarily for setting the field to an undocumented or not yet
         * supported value.
         */
        fun results(results: JsonField<List<PublicActor>>) = apply {
            this.results = results.map { it.toMutableList() }
        }

        /**
         * Adds a single [PublicActor] to [results].
         *
         * @throws IllegalStateException if the field was previously set to a non-list.
         */
        fun addResult(result: PublicActor) = apply {
            results =
        ...
        /** Alias for calling [addResult] with `PublicActor.ofAgent(agent)`. */
        fun addResult(agent: AgentActor) = addResult(PublicActor.ofAgent(agent))

        /** Alias for calling [addResult] with `PublicActor.ofBot(bot)`. */
        fun addResult(bot: BotActor) = addResult(PublicActor.ofBot(bot))

        /** Alias for calling [addResult] with `PublicActor.ofIntegrator(integrator)`. */
        fun addResult(integrator: IntegratorActor) = addResult(PublicActor.ofIntegrator(integrator))

        /** Alias for calling [addResult] with `PublicActor.ofSystem(system)`. */
        fun addResult(system: SystemActor) = addResult(PublicActor.ofSystem(system))

        /** Alias for calling [addResult] with `PublicActor.ofVisitor(visitor)`. */
        fun addResult(visitor: VisitorActor) = addResult(PublicActor.ofVisitor(visitor))

        /** Alias for calling [addResult] with `PublicActor.ofEmail(email)`. */
        fun addResult(email: EmailActor) = addResult(PublicActor.ofEmail(email))

        /** Alias for calling [addResult] with `PublicActor.ofLlm(llm)`. */
        fun addResult(llm: LlmActor) = addResult(PublicActor.ofLlm(llm))
        ...
        /**
         * Returns an immutable instance of [BatchResponsePublicActorWithErrors].
         *
         ...
         */
        fun build(): BatchResponsePublicActorWithErrors =
            BatchResponsePublicActorWithErrors(
    ...
    fun validate(): BatchResponsePublicActorWithErrors = apply {
        if (validated) {
            return@apply
        }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is BatchResponsePublicActorWithErrors &&
    ...
    override fun toString() =
        "BatchResponsePublicActorWithErrors{completedAt=$completedAt, results=$results, startedAt=$startedAt, status=$status, errors=$errors, links=$links, numErrors=$numErrors, requestedAt=$requestedAt, additionalProperties=$additionalProperties}"
}
