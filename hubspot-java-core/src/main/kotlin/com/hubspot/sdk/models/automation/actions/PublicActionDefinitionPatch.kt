...
private constructor(
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [PublicActionDefinitionPatch].
         */
        @JvmStatic fun builder() = Builder()
    }

    /** A builder for [PublicActionDefinitionPatch]. */
    class Builder internal constructor() {
        ...
        @JvmSynthetic
        internal fun from(publicActionDefinitionPatch: PublicActionDefinitionPatch) = apply {
            actionUrl = publicActionDefinitionPatch.actionUrl
            executionRules = publicActionDefinitionPatch.executionRules.map { it.toMutableList() }
            inputFieldDependencies =
                publicActionDefinitionPatch.inputFieldDependencies.map { it.toMutableList() }
            inputFields = publicActionDefinitionPatch.inputFields.map { it.toMutableList() }
            labels = publicActionDefinitionPatch.labels
            objectRequestOptions = publicActionDefinitionPatch.objectRequestOptions
            objectTypes = publicActionDefinitionPatch.objectTypes.map { it.toMutableList() }
            outputFields = publicActionDefinitionPatch.outputFields.map { it.toMutableList() }
            published = publicActionDefinitionPatch.published
        ...
        /**
         * Returns an immutable instance of [PublicActionDefinitionPatch].
         *
         * Further updates to this [Builder] will not mutate the returned instance.
         */
        fun build(): PublicActionDefinitionPatch =
            PublicActionDefinitionPatch(
                ...
                (inputFields ?: JsonMissing.of()).map { it.toImmutable() },
                labels,
                objectRequestOptions,
                (objectTypes ?: JsonMissing.of()).map { it.toImmutable() },
    ...
    fun validate(): PublicActionDefinitionPatch = apply {
        if (validated) {
            return@apply
        }
        ...
        inputFields().ifPresent { it.forEach { it.validate() } }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return other is PublicActionDefinitionPatch &&
    ...
    override fun toString() =
        "PublicActionDefinitionPatch{actionUrl=$actionUrl, executionRules=$executionRules, inputFieldDependencies=$inputFieldDependencies, inputFields=$inputFields, labels=$labels, objectRequestOptions=$objectRequestOptions, objectTypes=$objectTypes, outputFields=$outputFields, published=$published, additionalProperties=$additionalProperties}"
}
